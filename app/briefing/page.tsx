...
export default function BriefingPage() {
  ...
  const handleStart = () => {
    ...
    setTimeout(() => setShowBtn(true), 5000); // 5s Delay from success.html
  };
  ...
  return (
    <main className="bg-slate-950 text-white min-h-screen flex items-center justify-center p-6 relative">
      <div className="glass p-8 md:p-12 rounded-[40px] max-w-3xl w-full text-center shadow-2xl relative z-10 border border-white/10">
        ...
        <div className="relative aspect-video rounded-3xl overflow-hidden border border-white/10 bg-black mb-8 shadow-2xl">
            {!isPlaying && (
              <div className="absolute inset-0 bg-slate-900/98 z-20 flex flex-col items-center justify-center">
        ...
        <button 
          ...
          className={`inline-block bg-blue-600 px-10 py-5 rounded-2xl font-black uppercase text-sm tracking-widest hover:bg-blue-500 transition-all duration-700 shadow-lg shadow-blue-600/30 ${
